...
def resolve_indicator_by_id(_, info, id):
    ...
    indicator = [indicator for doc in response_json["docs"]
                 for indicator in doc["indicators"] if indicator["id"] == id]
    ...
    # logger.info("{process_name}:resolve_indicator_by_id: Found {n} indicators".format(process_name=PROCESS_NAME))
    logger.debug("FOUND")
    logger.debug(json.dumps(indicator, indent=4))
...
def resolve_entry_by_id(_, info, id):
    ...
    logger.debug(json.dumps(response_json, indent=4))
    try:
        entry = [doc["entry"] for doc in response_json["docs"] if doc["entry"]["id"] == id]

        logger.debug(json.dumps(entry[0], indent=4))
...
def resolve_impact(_,info,entryID, indicatorID):
    ...
    impact = {}
    try:
        impact["id"] = uuid.uuid1().int
        coefficient = [indicator["value"] for doc in response_json["docs"] for indicator in doc["indicators"] if indicator["id"] == indicatorID]
        
        if coefficient[0] != None:
            impact["coefficient"] = coefficient[0]
        else:
            impact["coefficient"] = 0.0

        impact["indicator"] = resolve_indicator_for_impact(impact, info=None, indicatorID=indicatorID)
        impact["entry"] = resolve_entry_for_impact(impact, info=None, entryID=entryID)
...
def resolve_indicator_for_impact(impact, info, indicatorID):
    ...
    indicator = resolve_indicator_by_id(None, None,indicatorID)
...
def resolve_entry_for_impact(impact, info, entryID):
    logger.debug(info)
    logger.debug("{process_name}:resolve_indicator_for_impact(): Resolving indicator for impact {impact}".format(
        process_name=PROCESS_NAME, impact=impact["id"]))

    entry = resolve_entry_by_id(None, None, entryID)

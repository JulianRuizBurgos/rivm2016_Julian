...
class DatabaseManager():
    ...
    def populate_database_from_csv(self, path, database_name):
        try:
            ...
            filter_fields = ["index", "Data source", "Ecoinvent process OR other names", "Unit", "Reference quantity", ""]
            for entry in parsed_json["data"]:
                # self.__logger.debug(json.dumps(entry, indent=4, sort_keys=True))
                refactored_entry_json = {}
                indicators = []
                for key, value in entry.items():
                    if key not in filter_fields:
                        ...
                        indicators.append({ str(id): indicator})
                    else:
                        refactored_entry_json[key] = value

                refactored_entry_json["indicators"] = indicators
                # self.__logger.debug(json.dumps(refactored_entry_json, indent=4, sort_keys=True))

                self.__add_element_to_database(database_name, refactored_entry_json)
